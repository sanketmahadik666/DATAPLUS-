#!/usr/bin/env python3
"""
Threaded Fast Real PDF Metadata Extractor
...
class ThreadedFastMetadataExtractor:
    """Threaded fast metadata extractor with maximum CPU utilization"""
    ...
    def analyze_all_pdfs(self, pdf_folder: str) -> Dict[str, Any]:
        ...
        logger.info(f"Starting THREADED FAST metadata extraction from PDFs in: {pdf_folder}")
        ...
        logger.info(f"THREADED FAST extraction completed: {successful}/{len(pdf_files)} successful in {total_time:.2f}s")
        logger.info(f"Processing rate: {len(pdf_files)/total_time:.1f} files/second")
        
        return self.results
    
    def save_results(self, output_file: str = "threaded_fast_pdf_metadata.json"):
    ...
    def generate_ml_dataset(self, output_file: str = "threaded_fast_ml_dataset.json"):
...
def main():
    ...
    print("=" * 80)
    print("THREADED FAST REAL PDF METADATA EXTRACTOR")
    print("=" * 80)
    
    # Initialize threaded fast extractor
    extractor = ThreadedFastMetadataExtractor()
    ...
    print(f"Starting THREADED FAST metadata extraction from all PDFs in: {pdf_folder}")
    ...
    # Run threaded fast metadata extraction
    results = extractor.analyze_all_pdfs(pdf_folder)
    
    # Save results
    extractor.save_results("threaded_fast_pdf_metadata.json")
    
    # Generate ML training dataset
    ml_dataset = extractor.generate_ml_dataset("threaded_fast_ml_dataset.json")
    
    # Display summary
    metadata = results["metadata"]
    print("\n" + "=" * 80)
    print("THREADED FAST METADATA EXTRACTION COMPLETE!")
    print("=" * 80)
    ...
    print("\nFiles generated:")
    print("  - threaded_fast_pdf_metadata.json (complete real metadata)")
    print("  - threaded_fast_ml_dataset.json (ML training dataset)")
